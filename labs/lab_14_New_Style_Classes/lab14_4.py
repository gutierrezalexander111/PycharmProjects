#!/usr/bin/env python
"""
lab14_4.py A Veggie class hierarchy, with a class method
and a staticmethod.
"""
...
class Veggie:
    ...
    def PredictHarvest(self):
		...
		Veggie.
        """
        harvest_index = self.weather_d[self.weather] - \
		  self.bug_index
    ...
    def UpdateBugIndex(cls, bug_index):
        ...
        """
        if cls == Veggie:
            raise RuntimeError, "Veggie must in instantiated " + \
			  "in a subclass."
    ...
    def UpdateWeather(weather):
        ...
        if weather not in Veggie.weather_d:
            raise ValueError, "weather must %s" % ', '.join(
				possible_weather)
