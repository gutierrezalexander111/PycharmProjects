#!/usr/bin/env python
"""Swapping cats and dogs again, but this time doing a 
better job withregular expressions.
"""
import re 
...
def DoSwap(text, apple, orange):
    ...
    def SwapMatch(match_object):
        ...
        if found.lower() == apple.lower():
            return MatchCase(orange, found) \
                   + match_object.group('plural')
        return MatchCase(apple, found) \
               + match_object.group('plural')
...
"""
$ re_swap.py
DOGS: 14elephants are a lot of Elephants.
3 Categories of DOGS and cats.
$"""
